...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfileFromFirestore = async () => {
      try {
        const docRef = doc(db, "users", user.uid); // Change "users" to your actual collection name
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const userData = docSnap.data();
          console.log("User data from Firestore:", userData);
          // Optional: setUserName(userData.name); // if you want to use this name
        } else {
    ...
    // Handle location state or fallback to localStorage
    if (location.state?.profileID && location.state?.name) {
      setUserName(location.state.name);
      setProfileID(location.state.profileID);
      localStorage.setItem("currentProfileID", location.state.profileID);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <div style={styles.iconsContainer}>
          <IconBox icon="❤️" label="Favorites" path="/wishlist" />
          <IconBox icon="👤" label="Profile" path="/profile" />
        </div>
      </div>
